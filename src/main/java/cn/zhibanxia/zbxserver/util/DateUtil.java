...
/**
 * 时间格式化工具类
 * Created by zzy on  2018/10/04 21:48
 */
public class DateUtil {
    ...
    public static boolean equalsInTimeString(String ts1, String ts2) {
        if (ts1.equals(ts2))
            return true;
        if (StringUtils.equals(ts1, "00:00") || StringUtils.equals(ts1, "24:00")) {
            if (StringUtils.equals(ts2, "00:00") || StringUtils.equals(ts2, "24:00")) {
                return true;
            }
        }
        return false;
     ...
     * @return xx
     */
    public static Calendar string2calendar(String timeString) {
        if (!DateUtil.isTimeString(timeString))
            throw new IllegalArgumentException("Wrong argument : timeString format error " + timeString);
     ...
     * @return
     */
    public static String dealDate(Date smdate) {
        Date now = new Date();
        int result = daysBetween(smdate, now);
        if(result == 0)
            return "今天";
        else if(result == 1)
            return "昨天";
        else
            return getDayStr(smdate);
    }
