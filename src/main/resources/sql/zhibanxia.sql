...
subdistrict_id			tinyint(32)		not null				comment '街道id',
area_detail				varchar(128)	default null			comment '详细地址',
gmt_create      		datetime        DEFAULT CURRENT_TIMESTAMP	comment '创建时间',
gmt_modified    		datetime        DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP	comment '修改时间',
primary key (id),
...
drop table if exists tb_recyle_request;
create table tb_recyle_request(
id              		bigint(20)  	unsigned NOT NULL auto_increment 	comment '主键自增',
create_user_id   		bigint(20)    	not null 				comment '发布用户id',
recyle_user_id     		bigint(20)  	default null			comment '回收人员id，确认回收后填充',
res_type				tinyint(2)		not null   				comment '回收资源类型：1.纸板；2.塑料瓶; 3.纸板和塑料瓶',
res_status				tinyint(2)		not null  				comment '回收资源状态：1.已发布；2.已确认待回收；3.已确认已回收；4.删除',
...
comfirm_recyle_time		datetime		default null 			comment '确认回收时间',
complete_recyle_time	datetime		default null 			comment '完成回收时间',
gmt_create      		datetime        DEFAULT CURRENT_TIMESTAMP	comment '创建时间',
gmt_modified    		datetime        DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP	comment '修改时间',
primary key (id),
key inx_create (create_user_id,res_status),
key inx_recyle (recyle_user_id,res_status)
